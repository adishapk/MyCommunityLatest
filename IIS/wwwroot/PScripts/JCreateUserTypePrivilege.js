                    ...
                    rowGroup: {
                        ...
                        startRender: function (rows, group) {
                            ...
                            var uData = rows
                                .data();         
                            return $('<tr/>')
                                .append('<td colspan="' + rows.columns()[0].length + '">' + group + ' (' + rows.count() + ')<i onclick="Update(' + uData[0].userTypeId + ')"  class="fas fa-pencil-alt" style="padding-left:15px;float:right;color:orange;cursor: pointer;"></i><i onclick="Deletedata(' + uData[0].userTypeId + ' )" class="fas fa-trash" style="float:right;color:red;cursor: pointer;"></i></td>')
...
function checkDuplication(val) {
    ...
    var colm = "user_type_id";
        var condition = "user_type_id::varchar = :user_type_id and previlege_type='UserType'";
                ...
                for (var x = 0; x < data.length; x++) {
                    ...
                    if (data[x].masterParentId == 0 && data[x].masterSubParentId == 0) {
                        markup = '<div class="panel">'
                            ...
                            + '<i class="fa fa-plus-circle" style="color:green;padding-right:10px;font-size:22px;"></i><input type="checkbox" id="mainchk' + x + '" class="icheckbox_flat-green" value="' + data[0].regId + ',' + data[x].masterId + ',0,0,' + 'pr' + x + '' + ',0,0,' + data[x].fileName +'"; onclick=changechk(this,'+data[x].masterId+','+x+')>&nbsp;&nbsp;'
                        ...
                        markup1 = '<li id="li' + data[x].masterId +'"><p><div class="row">'
                            ...
                            + '<input type="checkbox" id="subchk' + x + '" class="icheckbox_flat-green" value="' + data[0].regId + ',' + data[x].masterId + ',' + data[x].masterParentId + ',0,0,' + 'pr' + x + '' + ',0,'+ data[x].fileName+'"; onclick=changechk(this,' + data[x].masterId + ',' + x +')>&nbsp;&nbsp;' + data[x].masterName + '</div>'
                        ...
                        markup2 = '<li id="subli' + data[x].masterId + '"><p><div class="row">'
                            ...
                            + '<div class="col-md-10 col-sm-10">'
                            + '<input type="checkbox" id="subsubchk' + x + '" class="icheckbox_flat-green"  value="' + data[0].regId + ',' + data[x].masterId + ',' + data[x].masterParentId + ',' + data[x].masterSubParentId +',0,0,' + 'pr' + x + '' + ',' + data[x].fileName +'"; onclick=changechk(this,' + data[x].masterId + ',' + x +')>&nbsp;&nbsp;' + data[x].masterName + '</div>'
                            + ' <div class="col-md-2 col-sm-2">'
            ...
            if ($("#prioStatus").val() == 0) {
                {
                    if (chkArray.length > 0) {
                        ...
                        for (var d in chkArray) {      
                            ...
                            Array.push({ "regId": regId, "userTypeId": userTypeId, "masterId": chkArray[d].masterId, "masterParentId": chkArray[d].masterParentId, "masterSubParentId": chkArray[d].masterSubParentId, "masterPriority": $("#" + masterpr + " option:selected").val(), "masterSubPriority": $("#" + subpr + " option:selected").val(), "masterSubSubPriority": $("#" + subsubpr + " option:selected").val(), "fileName": chkArray[d].fileName, "userId": 0 })
                    ...
                    if (btn == "Submit") {
                        ...
                        statusmsg = 'UserType Privilege Successfully Added!!'
                    }
                    else {
                        url1 = "/CreateUserTypePrivilege/UpdateUserTypePrivilege";
                        statusmsg = 'UserType Privilege Successfully Updated!!'
                    }
                    if (Array.length > 0) {
                        $.ajax({
                            type: 'POST',
                            data: { userTypes: Array, count: cnt, previlege_type: "UserType" },
                            url: url1,
                            dataType: 'json',
                            success: function (data) {
                                if (data == false) {
    ...
    swalWithBootstrapButtons.fire({
        ...
        if (result.value) {
            $.ajax({
                ...
                data: { Id: id, previlege_type: "UserType" },
                datatype: "json",
                success: function (data) {                    
                    GetAll();                  
                },
...
function SetRoles(_val) {
    $.ajax({
        type: "POST",
        url: "CreateUserPrivilege/GetSelectedUserTypePrvileges",
        data: { userTypeId: _val, previlege_type: "UserType"},
        datatype: "json",
        success: function (data) {
            if (data != "") {
